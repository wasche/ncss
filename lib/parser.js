module.exports = function(outputBuffer, options) {
  ...
  function collapseZeroes() {
    ...
    } else if (t === 'none' && (NONE_PROPERTIES.indexOf(property) !== -1 || property === 'background') && !options.noCollapseNone) {
      buffer('0');
    } else {
      buffer(t);
    }
  ...
  function handle(type, token) {
    ...
    // first-letter and first-line must be followed by a space
      ...
      if (pending === ';'){
        if (options.keepTrailingSemicolons){
          buffer( ';' );
        }
        pending = '}';
      }
      ...
      } else if (token === 'none' && prev === ':' && NONE_PROPERTIES.indexOf(property) !== -1 && !options.noCollapseNone) {
        q('0');
